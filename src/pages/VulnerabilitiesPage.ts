...
export class VulnerabilitiesPage extends BasePage {
  protected url = "http://35.74.242.225:20010/dashboard/vulnerabilities";
  ...
  async clickVulnerabilitiesMenu() {
    ...
    const vulnMenu = this.page.locator('[role="menuitem"]').filter({ hasText: /^(safety )?Vulnerabilities$/  }).or(
      this.page.locator('[role="menuitem"]').filter({ hasText: /^(safety )?脆弱性$/ })
    );
